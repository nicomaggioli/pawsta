...
function Announce() {
  const items = [
    ...
    "100% natural rubber, made in Brooklyn",
    "Vet-tested. Dog-approved. Lawyer-cleared.",
    "Subscribe & save 15%",
    "Gift wrapping (with twine, obviously)"
  ];
      ...
      <div className="hero-inner">
        <div>
          <div className="hero-eyebrow">
            <span className="flag"><span className="g"/><span className="w"/><span className="r"/></span>
            The World's #1 Brand of Dog Chews
          </div>
          <h1>
            <span className="yellow">Al</span> <span className="stroke">dente.</span><br/>
            <span className="red">Al</span> <span>dog.</span>
          </h1>
          <p className="hero-sub">
            Rubber chew toys shaped like the pasta you grew up with. Engineered for molars, not marinara. Nine shapes. One very satisfied dog.
          </p>
          <div className="hero-ctas">
            <button className="btn btn-primary" onClick={onShop}>Shop the shapes →</button>
            <button className="btn btn-ghost" onClick={() => document.getElementById("story").scrollIntoView({ behavior: "smooth" })}>Our story</button>
          </div>
          <div className="hero-stats">
            <div className="hero-stat"><span className="num">9</span><span className="label">Pasta Shapes</span></div>
            <div className="hero-stat"><span className="num">100%</span><span className="label">Natural Rubber</span></div>
        ...
        <div className="hero-pack">
          <img className="pack-1" src="assets/Penne-cutout.png" alt="Penne Chew package"/>
          <img className="pack-2" src="assets/Ravioli-cutout.png" alt="Ravioli Chew package"/>
        </div>
      </div>
...
// === STRIPE BANNER ===
function Stripe() {
  const items = ["Made for chewers", "Not a food product", "Squeaks included", "Vet-tested", "Brooklyn-built"];
  const star = (
...
function ProductGrid({ onOpen, onQuickAdd }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (collection === "best") return all.filter(p => p.reviews > 800);
    if (collection === "small") return all.filter(p => p.sizes.includes("XS") || p.sizes.includes("S"));
    if (collection === "tough") return all.filter(p => p.durability >= 5);
    if (collection === "new") return all.filter(p => p.badge === "New");
    return all;
  }, [collection]);
    ...
    <section className="section" id="shop" style={{background: "var(--cream)"}}>
      <div className="section-inner">
        <div className="section-header">
          <div>
            <div className="section-eyebrow">The Shop · Nine Shapes</div>
            <h2 className="section-title">Pick your pasta.</h2>
          </div>
          <div className="tabs">
            {window.COLLECTIONS.map(c => (
              <button key={c.id} className={`tab ${collection === c.id ? "active" : ""}`} onClick={() => setCollection(c.id)}>
                {c.label}
              </button>
            ))}
          </div>
...
function Process() {
  const steps = [
    { n: "01", t: "Cooked Up", d: "Every shape starts as a sketch on a napkin in our Brooklyn kitchen-slash-workshop." },
    { n: "02", t: "Molded", d: "Food-grade molds press 100% natural rubber into pasta-perfect detail. Ridges, crinkles, all of it." },
    { n: "03", t: "Tested", d: "Our in-house panel of 14 dogs (and 3 cats with strong opinions) chew-tests every batch." },
    { n: "04", t: "Boxed", d: "Packed in recycled blister-card. Wrapped in twine. Shipped within 24 hours of approval." }
  ];
  return (
    ...
    <section className="section story" id="story">
      <div className="story-inner">
        <div className="story-image">
          ...
          <div className="stamp">
            <span className="est">Est. — Brooklyn</span>
            <span className="big">2021</span>
        ...
        <div className="story-text">
          ...
          <h2>A nonna, a dog, and a very chewed-up rolling pin.</h2>
          <p>
            It started when our beagle Tito chewed through a wooden rolling pin that had been in the family for forty-three years. We needed something he could destroy that wasn't an heirloom.
          </p>
          <p>
            Three years and one extremely patient mold-maker later, Pawsta was born. Each shape is engineered to the exact dimensions of its real-world counterpart, but cast in dense, food-grade rubber that takes a beating without giving up.
          </p>
          <p>
            We are dog people. We are pasta people. We are people who think a chew toy should look like something worth chewing.
          </p>
    ...
    <footer className="footer">
      <div className="footer-inner">
        <div className="footer-grid">
          <div>
            <Logo size={24}/>
            <div className="footer-tag">"Al dente. Al dog."<br/>— Tito, our beagle. Probably.</div>
            <div className="newsletter">
              <input type="email" placeholder="your@email.com"/>
              <button>Sign me up</button>
            </div>
            <div style={{fontSize: 11, opacity: 0.5, marginTop: 12, letterSpacing: "0.06em"}}>
              We email rarely. We promise. (Tito is in charge of the calendar.)
            </div>
          </div>
          <div>
            <h4>Shop</h4>
            <ul>
              <li><a>All Shapes</a></li>
              <li><a>Best Sellers</a></li>
              <li><a>Small Dogs</a></li>
              <li><a>Tough Chewers</a></li>
        ...
        <div className="footer-bottom">
          <span>© 2026 Pawsta Co. · Brooklyn, NY · 100% Natural Rubber</span>
          <span>Not a food product · Supervise your pet · Made on Earth</span>
        </div>
      </div>
    </footer>
  );
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="option-group">
            <div className="option-label"><span>Flavor</span><span className="picked">{flavor}</span></div>
            <div className="option-pills">
          ...
          <dl className="modal-meta">
            ...
            <div><dt>Avg. Chew Time</dt><dd>{product.chewTime} min/session</dd></div>
...
Object.assign(window, {
  ...
  ProductCard, ProductGrid, Process, Story, Reviews, Footer,
